...
Data=Data(del,:);
for k=1:Num_signal
    Autospectrum(:,:,k)=Data(length(Data)/Num_signal*(k-1)+1:length(Data)/Num_signal*k,:);
end
%% Autospectrum time
...
Data=Data(del,:);
for k=1:Num_signal
    Autospectrum_time(:,:,k)=Data(length(Data)/Num_signal*(k-1)+1:length(Data)/Num_signal*k,:);
end
...
Data=Data(del,:);
for k=1:Num_signal
    Time_Cap_compressed(:,:,k)=Data(length(Data)/Num_signal*(k-1)+1:length(Data)/Num_signal*k,:);
end
...
for k=1:Num_signal
    for l=1:size(Data,2)-2; 
        Time_Cap(k,1024*(l-1)+1:1024*l)=Data(1024*(k-1)+1:1024*k,l+2);
    end
end
dt=Data(2,2);
